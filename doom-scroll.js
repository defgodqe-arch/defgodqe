/* defgodqe — Doom Scroll: TikTok embed test mode */
(function(){
'use strict';
if(window.__defgodqeDoomScroll)return;
window.__defgodqeDoomScroll=true;

// Official TikTok Embed Player test posts. These stay hosted by TikTok; the app does not download them.
const TIKTOK_POSTS=[
  '6718335390845095173',
  '7080213458555737986',
  '7080217258545735586',
...
playerWrap.innerHTML='<iframe id="dfDoomTikTok" title="TikTok video" allow="autoplay; encrypted-media; fullscreen; picture-in-picture" scrolling="no" referrerpolicy="strict-origin-when-cross-origin"></iframe><div id="dfDoomTikTokFallback">TikTok could not load this embed.<br><a id="dfDoomTikTokLink" target="_blank" rel="noopener noreferrer">Open video on TikTok</a></div>';
overlay.appendChild(playerWrap);
const player=playerWrap.querySelector('#dfDoomTikTok');
const fallback=playerWrap.querySelector('#dfDoomTikTokFallback');
const fallbackLink=playerWrap.querySelector('#dfDoomTikTokLink');
...
function loadActive(){
  ...
  fallbackLink.href='https://www.tiktok.com/player/v1/'+id;
  setTimeout(()=>{if(isOpen&&token===loadToken)fallback.style.display='block'},5000);
}
...
feed.addEventListener('wheel',e=>{if(!isOpen)return;e.preventDefault();if(wheelLocked)return;wheelLocked=true;goToVideo(e.deltaY>0?1:-1);setTimeout(()=>wheelLocked=false,260)},{passive:false});
