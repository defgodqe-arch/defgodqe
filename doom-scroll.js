/* defgodqe — 1000-video randomized Doom Scroll */
(function(){
'use strict';
if(window.__defgodqeDoomScroll)return;
window.__defgodqeDoomScroll=true;

// Zack D. Films has thousands of uploads; YouTube's uploads playlist lets us address
// individual videos without shipping 1000 IDs or requiring a YouTube API key.
// We keep exactly one YouTube player alive, so the 1000-video feed stays lightweight.
const SOURCE_PLAYLIST='UUvz84_Q0BbvZThy75mbd-Dg';
...
.df-doom-card{position:relative;height:100dvh;width:100%;scroll-snap-align:start;scroll-snap-stop:always;display:flex;align-items:center;justify-content:center;background:radial-gradient(circle at 50% 35%,#202000 0,#090b10 38%,#030407 100%);contain:layout paint}
.df-doom-player{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;pointer-events:none}.df-doom-video{width:min(100vw,56.25dvh);height:min(100dvh,177.78vw);border:0;background:#000;box-shadow:0 0 45px rgba(250,204,21,.16);pointer-events:auto}
...
function srcFor(position){
 ...
 return `https://www.youtube-nocookie.com/embed?listType=playlist&list=${SOURCE_PLAYLIST}&index=${idx}&autoplay=1&mute=0&playsinline=1&rel=0&enablejsapi=1&vq=hd720`;
}
function stopPlayer(){playToken++;try{if(frame.contentWindow)frame.contentWindow.postMessage(JSON.stringify({event:'command',func:'stopVideo',args:[]}),'https://www.youtube-nocookie.com')}catch(_){}frame.removeAttribute('src');lastLoaded=-1}
function playAt(position){
 ...
 setTimeout(play,120);setTimeout(play,550);setTimeout(play,1100);
}
...
overlay.addEventListener('click',e=>{
 const o=e.target.closest('[data-open]');if(o)window.open('https://www.youtube.com/playlist?list='+SOURCE_PLAYLIST,'_blank','noopener');
