/* defgodqe — visible randomized Doom Scroll */
(function(){
'use strict';
...
.df-doom-player{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;z-index:99991;pointer-events:none;background:#030407}
.df-doom-video{display:block;width:min(100vw,56.25dvh);height:min(100dvh,177.78vw);min-width:320px;min-height:180px;border:0;background:#000;box-shadow:0 0 45px rgba(250,204,21,.16);pointer-events:auto;visibility:visible;opacity:1}
.df-doom-shade{position:fixed;inset:0;pointer-events:none;background:linear-gradient(180deg,rgba(0,0,0,.25),transparent 25%,transparent 62%,rgba(0,0,0,.7));z-index:99992}
...
.df-doom-actions{position:absolute;right:18px;bottom:28px;display:flex;flex-direction:column;gap:12px;z-index:99994}.df-doom-action,.df-doom-close{width:48px;height:48px;border:1px solid rgba(250,204,21,.45);border-radius:50%;background:rgba(10,13,20,.78);color:#fff;display:grid;place-items:center;cursor:pointer;backdrop-filter:blur(12px);font-size:20px}.df-doom-close{position:fixed;top:18px;right:18px;z-index:99995}.df-doom-top{position:fixed;top:20px;left:22px;z-index:99995;font-weight:900;letter-spacing:.08em;color:#facc15;text-shadow:0 0 16px rgba(250,204,21,.5)}
.df-doom-count{position:fixed;top:52px;left:22px;z-index:99995;color:#a1a1aa;font-size:11px;font-weight:700}
...
@media(max-width:600px){.df-doom-info{bottom:24px;left:14px;right:74px}.df-doom-actions{right:12px;bottom:24px}.df-doom-title{font-size:17px}.df-doom-video{width:100vw;height:100dvh;min-width:0;min-height:0;object-fit:contain}}
`;
...
// Keep the player OUTSIDE the scrolling container so it is always visible.
const playerWrap=document.createElement('div');playerWrap.className='df-doom-player';
playerWrap.innerHTML='<iframe id="dfDoomVideo" class="df-doom-video" title="Doom Scroll video" allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>';
...
function playAt(position){
 ...
 if(lastLoaded!==position){
   ...
   frame.src=srcFor(position);
 }
 // Browsers can block scripted unmuted autoplay. Start muted so the video is actually visible,
 // then let the user's click/tap on the player enable sound.
 setTimeout(()=>{if(!isOpen||activeIndex!==position||token!==playToken)return;try{frame.contentWindow?.postMessage(JSON.stringify({event:'command',func:'playVideo',args:[]}),'https://www.youtube.com')}catch(_){}},500);
}
...
function userPlay(){if(!isOpen||activeIndex<0)return;try{const p=(func,args=[])=>frame.contentWindow?.postMessage(JSON.stringify({event:'command',func,args}),'https://www.youtube.com');p('playVideo');p('unMute');p('setVolume',[100])}catch(_){} }
...
let scrollRaf=0;
feed.addEventListener('scroll',()=>{if(scrollRaf)return;scrollRaf=requestAnimationFrame(()=>{scrollRaf=0;if(!isOpen)return;const i=Math.max(0,Math.min(VIDEO_COUNT-1,Math.round(feed.scrollTop/innerHeight)));if(i!==activeIndex){stopPlayer();playAt(i)}})},{passive:true});
frame.addEventListener('load',()=>{if(isOpen&&activeIndex>=0){setTimeout(userPlay,300)}});
frame.addEventListener('error',()=>{frame.style.visibility='visible';frame.style.opacity='1'});
overlay.addEventListener('pointerdown',e=>{if(e.target===frame||e.target.closest('#dfDoomVideo'))userPlay()},{passive:true});
