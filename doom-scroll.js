/* defgodqe — Doom Scroll: clean fullscreen video-only feed */
(function(){
'use strict';
...
#dfDoomFeed{position:absolute;inset:0;width:100%;height:100%;overflow-y:auto;scroll-snap-type:y mandatory;overscroll-behavior:contain;scrollbar-width:none}
#dfDoomFeed::-webkit-scrollbar{display:none}
.df-doom-card{height:100dvh;width:100%;scroll-snap-align:start;scroll-snap-stop:always;background:#000}
.df-doom-player{position:fixed!important;inset:0!important;width:100vw!important;height:100dvh!important;display:flex!important;align-items:center!important;justify-content:center!important;z-index:99991!important;background:#000;pointer-events:none}
#dfDoomPlayer{display:block!important;width:100vw!important;height:100dvh!important;border:0!important;background:#000!important;pointer-events:auto!important;visibility:visible!important;opacity:1!important}
@media(max-width:600px){#dfDoomPlayer{width:100vw!important;height:100dvh!important}}
`;
...
function createPlayer(){
 ...
 ytPlayer=new YT.Player('dfDoomPlayer',{width:'100%',height:'100%',videoId:videoIdFor(Math.max(0,activeIndex)),playerVars:{autoplay:0,controls:1,playsinline:1,rel:0,enablejsapi:1,origin:location.origin},events:{onReady:function(){ytReady=true;if(isOpen&&activeIndex>=0)loadActive(true)},onError:function(){}}});
}
function loadActive(auto){
 if(!ytReady||!ytPlayer||activeIndex<0)return;
 const id=videoIdFor(activeIndex),token=++playToken;
 try{if(auto)ytPlayer.loadVideoById(id);else ytPlayer.cueVideoById(id);setTimeout(()=>{if(isOpen&&token===playToken&&auto)try{ytPlayer.playVideo()}catch(_){}},250)}catch(_){}
}
