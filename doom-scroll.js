...
/*
 * Add direct .mp4/.webm URLs here when you have permission to use them.
 * YouTube embeds are intentionally disabled so no YouTube UI or branding
 * can appear inside Doom Scroll.
 */
const VIDEO_SOURCES=[];
...
playerWrap.innerHTML='<video id="dfDoomVideo" playsinline webkit-playsinline preload="auto"></video>';
overlay.appendChild(playerWrap);
const doomVideo=playerWrap.querySelector('#dfDoomVideo');
let activeIndex=-1,isOpen=false,playToken=0,scrollTimer=0;
...
async function cacheVideo(url){
  ...
  try{
    ...
    if(existing){cacheWarm.add(url);return url}
    const response=await fetch(url,{mode:'cors',credentials:'omit',cache:'no-store'});
    if(!response.ok)return null;
    await cache.put(url,response.clone());cacheWarm.add(url);return url;
  }catch(_){return null}
}
...
async function loadActive(auto){
  ...
  if(!isOpen||token!==playToken)return;
  doomVideo.src=cached||url;
  doomVideo.load();
  if(auto)try{await doomVideo.play()}catch(_){}
...
function playAt(position,immediate){
  ...
  activeIndex=position;loadActive(true);
}
...
function goToVideo(direction){
  ...
  feed.scrollTop=next*Math.max(1,innerHeight);playAt(next,true);
}
doomVideo.addEventListener('ended',()=>{if(isOpen)goToVideo(1)});
function open(){
  isOpen=true;order=shuffle(slots.slice());activeIndex=0;playToken++;
  overlay.classList.add('df-open');document.body.style.overflow='hidden';feed.scrollTop=0;
  loadActive(true);
}
async function close(){
  isOpen=false;playToken++;clearTimeout(scrollTimer);
  try{doomVideo.pause()}catch(_){}
  doomVideo.removeAttribute('src');doomVideo.load();
  overlay.classList.remove('df-open');document.body.style.overflow='';
...
feed.addEventListener('scroll',()=>{
  if(!isOpen)return;clearTimeout(scrollTimer);
  scrollTimer=setTimeout(()=>{const i=getIndex();if(i!==activeIndex)playAt(i,true)},100);
},{passive:true});
let wheelLocked=false;
feed.addEventListener('wheel',e=>{
  if(!isOpen)return;e.preventDefault();if(wheelLocked)return;
  wheelLocked=true;goToVideo(e.deltaY>0?1:-1);setTimeout(()=>{wheelLocked=false},260);
},{passive:false});
...
function addSidebarButton(){
  ...
  const mini=sidebar.querySelector('#dfGameBtn')?.closest('.px-3.pb-2');if(mini)mini.after(wrap);else user.before(wrap);
  document.getElementById('dfDoomBtn').onclick=open;return true;
}
